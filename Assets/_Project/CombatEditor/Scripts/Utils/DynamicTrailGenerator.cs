...
{	
	public class DynamicTrailGenerator
	{
	    ...
	    public void UpdateAllUVTo_0_1()
	    {
	        ...
	        {
	            float left = (float)i / QuadCount;
	            float right = (float)(i + 1) / QuadCount;
	            var LeftDown = new Vector2(left, 0);
	            var LeftUp = new Vector2(left, 1);
	            var RightDown = new Vector2(right, 0);
	            var RightUp = new Vector2(right, 1);
