...
{	
	public class CombatInspector : EditorWindow
	{
	        ...
	        if (inspectedType == CombatEditor.InspectedType.AnimationConfig)
	        {
	            ...
	            if (combatEditor.CurrentGroupIndex < combatDatas.arraySize && combatEditor.CurrentGroupIndex >= 0)
	            {
	                ...
	                if (combatEditor.CurrentAbilityIndexInGroup < ObjsProperty.arraySize && combatEditor.CurrentAbilityIndexInGroup >= 0)
	                {
	                    ...
	                    if (TargetObj.objectReferenceValue != null)
	                    {
                            DrawRenameAbilityButton((AbilityScriptableObject)TargetObj.objectReferenceValue);
	    ...
	    public AbilityScriptableObject CreateAbilityScriptableObject()
	    {
	        ...
	        while (true)
	        {
	            // Keep incrementing the suffix while an asset with the same name exists.
	    ...
	    public void DrawRenameAbilityButton(AbilityScriptableObject currentAbilityObj)
	    {
            EditorGUI.BeginDisabledGroup(currentAbilityObj == null || currentAbilityObj.Clip == null || currentAbilityObj.name == currentAbilityObj.Clip.name);
            if (GUILayout.Button("Rename Config To Clip Name"))
            {
                combatEditor.OnRenameAbilityObjToClipName(currentAbilityObj);
            }
            EditorGUI.EndDisabledGroup();
	    }
	    ...
	    public void DrawAnimationClipSelector(AbilityScriptableObject CurrentAbilityObj)
	    {
	        ...
	        EditorGUI.BeginChangeCheck();
	        AnimationClip selectedClip = (AnimationClip)EditorGUILayout.ObjectField("Clip", CurrentAbilityObj.Clip, typeof(AnimationClip), false);
	        if (EditorGUI.EndChangeCheck())
	        {
	            CurrentAbilityObj.Clip = selectedClip;
	            EditorUtility.SetDirty(CurrentAbilityObj);
	            AssetDatabase.SaveAssets();
	        }
	    ...
	    public void SelectCombatConfig()
	    {
	        ...
	        if (combatEditor == null || combatEditor.SelectedController == null)
	        {
	            return;
	        }
