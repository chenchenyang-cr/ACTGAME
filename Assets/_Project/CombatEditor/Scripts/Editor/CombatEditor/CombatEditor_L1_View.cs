            ...
            for (int i = 0; i < SelectedController.CombatDatas.Count; i++)
            {
                ...
                if (DataGroup.IsFolded)
                {
                    for (int j = 0; j < DataGroup.CombatObjs.Count; j++)
                    {
                        ...
                        float toolButtonWidth = LineHeight;
                        Rect DataSelectRect = new Rect(AbilityRect.x, AbilityRect.y + HeightCounter * LineHeight, AbilityRect.width - toolButtonWidth - Width_Scroll, LineHeight);
                        Rect DataDeleteRect = new Rect(DataSelectRect.xMax, AbilityRect.y + HeightCounter * LineHeight, toolButtonWidth, LineHeight);
                        HeightCounter += 1;
