...
namespace CombatEditor
{
    public partial class CombatEditor
    {
        ...
        public bool IsEditingTransform(AbilityEventObj obj, CombatEditorTransformEditState.EditMode mode)
        {
            return CombatEditorTransformEditState.IsEditing(obj, mode);
        }

        public void ToggleTransformEditing(AbilityEventObj obj, CombatEditorTransformEditState.EditMode mode)
        {
            if (CombatEditorTransformEditState.IsEditing(obj, mode))
            {
                DisableTransformEditing();
                return;
            }

            if (CombatEditorTransformEditState.CurrentMode != CombatEditorTransformEditState.EditMode.None)
            {
                DisableTransformEditing();
            }

            CombatEditorTransformEditState.Set(obj, mode);

            if (mode == CombatEditorTransformEditState.EditMode.Position)
            {
                Tools.current = Tool.Move;
            }
            else if (mode == CombatEditorTransformEditState.EditMode.Rotation)
            {
